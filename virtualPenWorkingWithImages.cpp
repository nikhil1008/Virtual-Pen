...
{   //ThreadPool 
    ...
    //Mat mat1(size, CV_8UC1, Scalar(0));
    //mat1.at<uchar>(100, 100) = 1;
    //inputImages.push_back(mat1);
    //Mat mat2(size, CV_8UC1, Scalar(0));
    //mat2.at<uchar>(0, 0) = 1;
    //inputImages.push_back(mat2); 
    ...
    while (not procQueue.empty()) {
        auto fu = move(procQueue.front());
        procQueue.pop();
        auto status = fu.wait_for(chrono::milliseconds(20));
        if (status != future_status::ready) {
            cout << "Image ID: " << imgId << " Not Ready ! \n";
            procQueue.push(move(fu));
            imgId += 1;
            imgId %= 2;
        }
        else {
            auto doneProcessing = fu.get();
            if (doneProcessing)
            {
                imshow("test", board);
                waitKey(0);
