...
public:
    ...
    void displayContentFromFutureQueue(ThreadSafeQueue<future<bool>>* procQueue,  Mat& board) {
        string window_name = "Virtual Board";
        ...
        //        cout << "\done proecssing from display Manager\n";
        //        if (doneProcessing)
        //        {
        //            imshow("test", board);
        //            waitKey(0);
        ...
        while (not procQueue->empty()) {
            cout << "\n\n *** procQ Size inside display = " << procQueue->size() << "***\n";
            auto fu_ = move(procQueue->pop());
            if (not fu_) continue;
            cout << "***doneProcessing***\n";
            auto doneProcessing = (*fu_).get();
...
{   //ThreadPool 
    ...
    promise<bool> isEscape; 
    future<bool> stopDisplay = isEscape.get_future();

    //futures
    ThreadSafeQueue<future<bool>> * procQueue = new ThreadSafeQueue<future<bool>>();
    
    //DisplayThread
    thread displayThread(&BoardDisplayer::displayContentFromFutureQueue,
        boardDisplayer, procQueue, ref(board));
    ...
    for (auto& img : inputImages) {
        ...
        cout << "moved to procQueue\n";
        cout << "procQ Size = " << procQueue->size() << "\n";
    }
